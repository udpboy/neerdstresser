...
const RegisterPage = ({
  ...
  <div className={`${cardClass(theme)} w-full max-w-2xl md:ml-0 md:mr-auto ${navOpen ? "sm:ml-64" : ""}`}>
    ...
    <form onSubmit={onRegisterSubmit} className="space-y-4 px-5 py-6 sm:px-6">
      ...
      <div className="space-y-2">
        <label className="text-sm text-slate-300">{t(lang, "auth.confirmPassword")}</label>
        <input
          type="password"
          required
          ...
          className={inputClass(theme)}
          placeholder={t(lang, "auth.confirmPlaceholder")}
        />
      </div>
      <div className="space-y-2">
      ...
      >
        {loading ? t(lang, "auth.loading") : t(lang, "auth.submitRegister")}
      </button>
    </form>
  </div>
);
